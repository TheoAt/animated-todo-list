...
import AnimatedCheckbox from './animations/animated-checkbox'
import AnimatedTaskLabel from './animations/animated-task-label'
    ...
    return(
        ...
        >
            <HStack alignItems='center' w='full' px={4} py={2} bg={useColorModeValue('warmGray.50', 'primary.900')}>
                ...
                {isEditing ? (
                    <Input placeholder='Tâche à réaliser' value={subject} variant="unstyled" fontSize={18} px={1} py={0} autoFocus blurOnSubmit onChange={handleChangeSubject} onBlur={onFinishEditing} />
                ) : (
                    <AnimatedTaskLabel strikethrough={isDone} textColor={activeTextColor} inactiveTextColor={doneTextColor} onPress={onPressLabel}>
                        {subject}
                    </AnimatedTaskLabel>
                )}
            </HStack>
